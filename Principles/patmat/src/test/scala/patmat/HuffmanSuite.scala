...
class HuffmanSuite extends FunSuite {
  ...
  test("Test big char set") {
    new TestTrees {
      val mt = createCodeTree("aaabbcccccdddddaaefggeeffffhh".toList)
      assert(decode(mt, quickEncode(mt)("abhf".toList)) === "abhf".toList)
      assert(decode(mt, encode(mt)("abdegh".toList)) === "abdegh".toList)
    }
  }
}
